<template>
	<view class="container">
		...
		<!-- <view class="nav-bar">
			...
			<text class="nav-title">{{ $t('wallet.rechargeTitle') }}</text>
		</view> -->
		...
		<view class="balance-section">
			<view class="balance-item">
				<view class="balance-left">
					...
					<text class="balance-text">{{ $t('wallet.balance') }}</text>
				</view>
		...
		<view class="recharge-section">
			<view class="recharge-item">
				<view class="recharge-left">
					...
					<text class="recharge-text">{{ $t('wallet.recharge') }}</text>
				</view>
			</view>
		</view>
		...
		<view class="input-section">
			<!-- 汇率 -->
			<view class="exchange-rate">{{ $t('wallet.exchangeRate') }}</view>
			...
			<view class="recharge-button" @click="doRecharge">
				<text class="recharge-button-text">{{ $t('wallet.rechargeButton', { amount: rechargeAmount }) }}</text>
			</view>
			
			<!-- 协议文本 -->
			<view class="agreement-text">
				{{ $t('wallet.agreementText') }}
				<text class="agreement-link" @click="goToServiceAgreement">{{ $t('wallet.serviceAgreement') }}</text>
				<text class="agreement-link" @click="goToMinorRefund">{{ $t('wallet.minorRefund') }}</text>
			</view>
		</view>
		...
		<view class="record-section">
			<view class="record-header">
				<view class="record-left">
					...
					<text class="record-text">{{ $t('wallet.rechargeRecord') }}</text>
				</view>
			</view>
			...
			<view class="record-item" v-for="record in rechargeRecords" :key="record.id">
				...
				<view class="order-number">{{ $t('wallet.orderNumber', { number: record.orderNumber }) }}</view>
			</view>
		</view>
		
		<!-- 底部服务条款 -->
		<view class="bottom-section">
			<text class="service-terms" @click="goToServiceTerms">{{ $t('wallet.serviceTerms') }}</text>
		</view>
		...
		methods: {
			...
			async loadWalletData() {
				try {
					...
					const response = await uni.request({
						url: `${this.$baseUrl}/api/wallet/balance?userId=${this.userId}`,
						method: 'GET',
						header: {
			...
			async loadRechargeRecords() {
				try {
					...
					const response = await uni.request({
						url: `${this.$baseUrl}/api/wallet/recharge-records?userId=${this.userId}`,
						method: 'GET',
						header: {
							'Content-Type': 'application/json'
						}
			...
			async submitRechargeInfo(paymentMethod) {
				try {
					...
					const response = await uni.request({
						url: '${this.$baseUrl}/api/wallet/recharge',
						method: 'POST',
						data: {
							userId: this.userId,
...
<style>
	...
	/* 充值按钮 */
	.recharge-button {
		background: linear-gradient(135deg, var(--color-red, #FF6B6B), var(--color-pink, #FF8E8E));
		border-radius: 50rpx;
		padding: 30rpx 0;
		margin-bottom: 30rpx;
		box-shadow: 0 10rpx 25rpx var(--color-red-shadow, rgba(255, 107, 107, 0.3));
	}
	...
	.record-item {
		padding: 30rpx 40rpx;
		border-bottom: 1rpx solid var(--color-border-light, #f5f5f5);
	}
