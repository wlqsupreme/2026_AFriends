...
public class UserEntityController {
    ...
    public ResponseEntity<List<UserInfo_njj>> getMentionableFriends(@PathVariable Long userId) {
        try {
            // 获取用户好友关系列表
            List<UserFriendsRelationship_njj> friendships = 
                userEntityService.getUserFriendsRelationshipByUserIdFromMemory(userId);
    ...
    public ResponseEntity<Map<String, Object>> getUserBaseInfo(@PathVariable Long userId) {
        try {
            ...
            result.put("hasPhoneBound", userBase.getLoginTelAccount() != null && !userBase.getLoginTelAccount().isEmpty());
    ...
    public ResponseEntity<Map<String, Object>> updateUserPhone(@RequestBody Map<String, Object> requestData) {
        try {
            ...
            String operationType = requestData.get("operationType") != null ? 
                requestData.get("operationType").toString() : "bind";
            
            System.out.println("开始" + (operationType.equals("bind") ? "绑定" : "更换") + "手机号，用户ID: " + userId + ", 手机号: " + phoneNumber);
