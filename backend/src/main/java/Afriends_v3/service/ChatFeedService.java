...
public class ChatFeedService {
    ...
    private void refreshPostBaseCache() {
        ...
        try {
            ...
            // 分页配置：每页500条（根据数据库性能调整，建议500~1000条）
            int pageSize = 500;
            int currentPage = 1;
            ...
            do {
                try {
                    ...
                    throw e; // 抛出异常，避免缓存部分数据
                }
            } while (currentPage <= novelPage.getPages());
            ...
            do {
                try {
                    imagePage = imagePostBaseMapper.selectPage(
                            new Page<>(currentPage, pageSize),
                            new QueryWrapper<ImagePostBase_wlq>().orderByDesc("created_at")
                    );
            ...
            } while (currentPage <= imagePage.getPages());
            ...
            do {
                try {
                    textPage = textpostBaseMapper.selectPage(
                            new Page<>(currentPage, pageSize),
                            new QueryWrapper<TextpostBase_wlq>().orderByDesc("created_at")
                    );
            ...
            } while (currentPage <= textPage.getPages());
