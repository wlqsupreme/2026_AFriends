...
public class EFMPRTEntityService {
    ...
    public Map<String, Object> saveTaskComment(Map<String, Object> requestData) {
        ...
        try {
            ...
            String commentText = requestData.get("commentText") != null ? requestData.get("commentText").toString() : "";
            ...
            // 2. 校验任务是否存在（通过 AI 任务需求表 v2_ai_task_require）
            AiTaskRequire_wlq task = AiTaskRequire_list_wlq.getAiTaskRequireById(taskId);
            if (task == null) {
                result.put("success", false);
            ...
            if (stars == null || stars < 1 || stars > 5) {
                result.put("success", false);
                result.put("message", "评分星级必须在 1 到 5 之间");
                result.put("stars", stars);
                return result;
            }

            // 4. 校验评论内容（可选：如果业务要求非空则开启）
