...
public class UserFollowService extends ServiceImpl<UserFollowRelationshipMapper, UserFollowRelationship_zjx> {
    ...
    public boolean followUser(Long userId, Long followedUserId) {
        log.info("开始执行关注逻辑：userId={}, followedUserId={}", userId, followedUserId);
        
        // 1. 检查是否已关注（有效状态：isActive=1）
        QueryWrapper<UserFollowRelationship_zjx> activeQuery = new QueryWrapper<>();
        activeQuery.eq("user_id", userId)
                .eq("followed_user_id", followedUserId)
                .eq("is_active", 1);
        UserFollowRelationship_zjx activeFollow = this.getOne(activeQuery);
        if (activeFollow != null) {
            System.out.println("UserFollowService: 用户[" + userId + "]已关注[" + followedUserId + "]");
            return true; // 已关注，直接返回true
        }

        // 2. 检查是否存在不活跃的关注关系（之前取消过关注）
        QueryWrapper<UserFollowRelationship_zjx> inactiveQuery = new QueryWrapper<>();
        inactiveQuery.eq("user_id", userId)
                .eq("followed_user_id", followedUserId)
                .eq("is_active", 0);
        UserFollowRelationship_zjx inactiveFollow = this.getOne(inactiveQuery);
        
        if (inactiveFollow != null) {
            // 3. 如果存在不活跃的关注关系，则更新为活跃状态（复用现有记录）
            System.out.println("UserFollowService: 发现不活跃的关注关系，重新激活");
            inactiveFollow.setIsActive(1); // 重新激活
            inactiveFollow.setAction("follow"); // 更新action
            inactiveFollow.setUpdatedAt(new Timestamp(System.currentTimeMillis()));
            
            boolean updateResult = this.updateById(inactiveFollow);
            if (updateResult) {
                // 更新内存缓存
                UserFollowRelationship_list_zjx.updateInCache(inactiveFollow);
                System.out.println("UserFollowService: 用户[" + userId + "]成功重新关注[" + followedUserId + "]");
                return true;
            } else {
                System.err.println("UserFollowService: 重新激活关注关系失败");
                return false;
            }
        }

        // 4. 如果不存在任何关注关系，则创建新记录
