...
public class ReportService {
    ...
    public Map<String, Object> submitReport(Long userId, Long reportedUserId, Long contentId, 
        ...
        // 数据验证
        Map<String, Object> validationError = validateReportData(userId, categoryId, description, reportedUserId, contentId);
     ...
     * 验证举报数据
     */
    private Map<String, Object> validateReportData(Long userId, Long categoryId, String description, 
            Long reportedUserId, Long contentId) {
        // 验证用户ID
        if (userId == null || userId <= 0) {
            Map<String, Object> error = new HashMap<>();
            error.put("success", false);
            error.put("message", "用户ID无效");
        ...
        if (categoryId == null || categoryId < 1 || categoryId > 10) {
            Map<String, Object> error = new HashMap<>();
            error.put("success", false);
            error.put("message", "举报类别ID无效，有效范围为1-10");
            error.put("timestamp", System.currentTimeMillis());
            return error;
        }
        ...
        if (description.length() > 1000) {
            Map<String, Object> error = new HashMap<>();
            error.put("success", false);
            error.put("message", "举报描述长度不能超过1000个字符");
            error.put("timestamp", System.currentTimeMillis());
            return error;
        }
        
        // 验证至少要有被举报用户ID或内容ID之一
        if (reportedUserId == null && contentId == null) {
            Map<String, Object> error = new HashMap<>();
            error.put("success", false);
            error.put("message", "必须提供被举报用户ID或内容ID");
            error.put("timestamp", System.currentTimeMillis());
            return error;
        }
