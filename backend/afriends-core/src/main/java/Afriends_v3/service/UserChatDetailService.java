...
public class UserChatDetailService {
    ...
    public Long saveChatDetail(Map<String, Object> chatData) {
        try {
            ...
            // 校验 sessionId 合法性（必须是数字，且 > 0）
            BigInteger sessionId = null;
            if (sessionIdObj == null) {
                System.err.println("UserChatDetailService: sessionId 不能为空");
                return null;
            }
            try {
                long sid = Long.parseLong(sessionIdObj.toString());
                if (sid <= 0) {
                    System.err.println("UserChatDetailService: sessionId 必须大于0, 实际值=" + sid);
                    return null;
                }
                sessionId = BigInteger.valueOf(sid);
            } catch (NumberFormatException e) {
                System.err.println("UserChatDetailService: sessionId 非法，必须是数字: " + sessionIdObj);
                return null;
            }

            // 校验 demandParty（发送方）必须为非空数字字符串
            if (demandPartyObj == null || demandPartyObj.toString().trim().isEmpty()) {
                System.err.println("UserChatDetailService: demandParty 不能为空");
                return null;
            }
            String demandParty = demandPartyObj.toString().trim();
            if (!demandParty.matches("\\d+")) {
                System.err.println("UserChatDetailService: demandParty 非法，必须是纯数字: " + demandParty);
                return null;
            }

            // 校验 responseParty（接收方）必须为非空数字字符串
            if (responsePartyObj == null || responsePartyObj.toString().trim().isEmpty()) {
                System.err.println("UserChatDetailService: responseParty 不能为空");
                return null;
            }
            String responseParty = responsePartyObj.toString().trim();
            if (!responseParty.matches("\\d+")) {
                System.err.println("UserChatDetailService: responseParty 非法，必须是纯数字: " + responseParty);
                return null;
            }

            // 校验 message 非空
            if (messageObj == null || messageObj.toString().trim().isEmpty()) {
                System.err.println("UserChatDetailService: message 不能为空");
                return null;
            }
            String message = messageObj.toString();

            // senderType 可以为空，但如果有则去掉首尾空格
            String senderType = senderTypeObj != null ? senderTypeObj.toString().trim() : null;
            ...
            chatDetail.setMessage(message);
            chatDetail.setSenderType(senderType);
