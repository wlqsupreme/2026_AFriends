...
public class UserEntityService {
    ...
    public Map<String, Object> saveUserFriendsSettings(Map<String, Object> settingsData) {
        try {
            ...
            String settingValue = settingsData.get("settingValue") != null ? settingsData.get("settingValue").toString() : null;

            // 1. 验证 settingId 是否有效（有效值：10000002=消息免打扰, 10000003=置顶聊天, 10000004=提醒, 10000005=聊天背景）
            Set<Long> validSettingIds = Set.of(10000002L, 10000003L, 10000004L, 10000005L);
            if (!validSettingIds.contains(settingId)) {
                System.err.println("UserEntityService: 无效的 settingId，settingId=" + settingId);
                Map<String, Object> errorResponse = new HashMap<>();
                errorResponse.put("success", false);
                errorResponse.put("message", "无效的设置ID，有效值为：10000002（消息免打扰）、10000003（置顶聊天）、10000004（提醒）、10000005（聊天背景）");
                errorResponse.put("settingId", settingId);
                errorResponse.put("timestamp", System.currentTimeMillis());
                return errorResponse;
            }

            // 2. 验证 settingValue 不能为空
            if (settingValue == null || settingValue.trim().isEmpty()) {
                System.err.println("UserEntityService: settingValue 不能为空");
                Map<String, Object> errorResponse = new HashMap<>();
                errorResponse.put("success", false);
                errorResponse.put("message", "设置值不能为空");
                errorResponse.put("timestamp", System.currentTimeMillis());
                return errorResponse;
            }

            // 3. 验证 settingValue 格式和值
            try {
                // 尝试解析 JSON 以验证格式
                com.fasterxml.jackson.databind.JsonNode jsonNode = OBJECT_MAPPER.readTree(settingValue);
                
                if (!jsonNode.isArray()) {
                    System.err.println("UserEntityService: settingValue 必须是 JSON 数组格式: " + settingValue);
                    Map<String, Object> errorResponse = new HashMap<>();
                    errorResponse.put("success", false);
                    errorResponse.put("message", "设置值必须是 JSON 数组格式");
                    errorResponse.put("settingValue", settingValue);
                    errorResponse.put("timestamp", System.currentTimeMillis());
                    return errorResponse;
                }
                
                // 根据 settingId 验证 settingValue 的有效值
                if (settingId == 10000002L || settingId == 10000003L || settingId == 10000004L) {
                    // 消息免打扰、置顶聊天、提醒：只允许 ["0"] 或 ["1"]
                    if (jsonNode.size() != 1 || !jsonNode.get(0).isTextual()) {
                        System.err.println("UserEntityService: settingValue 格式错误，应为包含单个字符串的数组: " + settingValue);
                        Map<String, Object> errorResponse = new HashMap<>();
                        errorResponse.put("success", false);
                        errorResponse.put("message", "设置值格式错误，应为包含单个字符串的数组，如 [\"0\"] 或 [\"1\"]");
                        errorResponse.put("settingValue", settingValue);
                        errorResponse.put("timestamp", System.currentTimeMillis());
                        return errorResponse;
                    }
                    
                    String value = jsonNode.get(0).asText();
                    if (!value.equals("0") && !value.equals("1")) {
                        System.err.println("UserEntityService: settingValue 值无效，只允许 \"0\" 或 \"1\": " + settingValue);
                        Map<String, Object> errorResponse = new HashMap<>();
                        errorResponse.put("success", false);
                        errorResponse.put("message", "设置值无效，只允许 [\"0\"] 或 [\"1\"]");
                        errorResponse.put("settingValue", settingValue);
                        errorResponse.put("timestamp", System.currentTimeMillis());
                        return errorResponse;
                    }
                } else if (settingId == 10000005L) {
                    // 聊天背景：允许 [""] 或包含字符串的数组（图片URL）
                    if (jsonNode.size() != 1 || !jsonNode.get(0).isTextual()) {
                        System.err.println("UserEntityService: settingValue 格式错误，应为包含单个字符串的数组: " + settingValue);
                        Map<String, Object> errorResponse = new HashMap<>();
                        errorResponse.put("success", false);
                        errorResponse.put("message", "设置值格式错误，应为包含单个字符串的数组");
                        errorResponse.put("settingValue", settingValue);
                        errorResponse.put("timestamp", System.currentTimeMillis());
                        return errorResponse;
                    }
                    // 聊天背景可以是空字符串或图片URL，这里只验证格式，不限制具体值
                }
            } catch (com.fasterxml.jackson.core.JsonProcessingException e) {
                System.err.println("UserEntityService: settingValue 格式无效，不是有效的 JSON: " + settingValue);
                Map<String, Object> errorResponse = new HashMap<>();
                errorResponse.put("success", false);
                errorResponse.put("message", "设置值格式无效，应为有效的 JSON 字符串");
                errorResponse.put("settingValue", settingValue);
                errorResponse.put("timestamp", System.currentTimeMillis());
                return errorResponse;
            }
            ...
            if (existingRelationship != null) {
                ...
                successResponse.put("message", "设置保存成功");
            ...
            System.err.println("UserEntityService: 参数格式错误: " + e.getMessage());
            Map<String, Object> errorResponse = new HashMap<>();
            errorResponse.put("success", false);
    ...
    public List<UserAiComment_njj> getAllUserAiCommentFromMemory() {
        ...
        if (!UserAiComment_list_njj.isCacheLoaded()) {
            System.out.println("UserEntityService: 检测到缓存未加载，自动加载用户AI评论数据...");
            try {
                loadUserAiCommentToMemory();
            } catch (Exception e) {
                System.err.println("UserEntityService: 自动加载用户AI评论数据失败: " + e.getMessage());
    ...
    public Map<String, Object> saveFriendsProfile(Map<String, Object> profileData) {
        try {
            ...
            UserBase_wlq user = userBaseMapper.selectById(userId);
            if (user == null) {
                System.err.println("UserEntityService: 用户不存在，userId=" + userId);
                Map<String, Object> errorResponse = new HashMap<>();
                errorResponse.put("success", false);
            ...
            // 2. 验证好友是否存在
            UserBase_wlq friend = userBaseMapper.selectById(friendId);
            if (friend == null) {
                System.err.println("UserEntityService: 好友不存在，friendId=" + friendId);
                Map<String, Object> errorResponse = new HashMap<>();
                errorResponse.put("success", false);
                errorResponse.put("message", "好友不存在");
                errorResponse.put("friendId", friendId);
                errorResponse.put("timestamp", System.currentTimeMillis());
                return errorResponse;
            }
