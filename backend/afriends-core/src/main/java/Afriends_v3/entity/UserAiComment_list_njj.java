...
public class UserAiComment_list_njj extends EntityList<UserAiComment_njj> {
    ...
    public static List<UserAiComment_njj> getUserAiCommentByAimodelId(Long aimodelId) {
        ...
        System.out.println("UserAiComment_list_njj: 查询 aimodelId=" + aimodelId + " 的评论");
        System.out.println("UserAiComment_list_njj: 缓存状态 - isCacheLoaded=" + isCacheLoaded + ", totalComments=" + totalUserAiComment);
        System.out.println("UserAiComment_list_njj: 缓存中的评论数量: " + userAiCommentCache.size());
        
        if (!isCacheLoaded || userAiCommentCache.isEmpty()) {
            System.out.println("UserAiComment_list_njj: 警告 - 缓存未加载或为空，无法查询评论");
            return result;
        }
        
        for (UserAiComment_njj comment : userAiCommentCache.values()) {
            if (comment != null && comment.getAimodelId() != null) {
                // 使用 Objects.equals 进行安全的比较
                if (comment.getAimodelId().equals(aimodelId)) {
                    result.add(comment);
                    System.out.println("UserAiComment_list_njj: 找到匹配的评论 - commentId=" + comment.getUserAiCommentId() + 
                                     ", aimodelId=" + comment.getAimodelId() + ", userId=" + comment.getUserId());
                }
            }
        }
        
        System.out.println("UserAiComment_list_njj: 查询完成，找到 " + result.size() + " 条评论");
        return result;
    }
