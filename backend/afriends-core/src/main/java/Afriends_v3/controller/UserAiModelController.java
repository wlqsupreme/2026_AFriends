...
public class UserAiModelController {
    ...
    public ResponseEntity<Map<String, Object>> updateAiModelLevel(@PathVariable Long userAiId,
        ...
        try {
            // 1. 获取并转换参数（支持Integer、Long、String等多种类型）
            Object levelObj = request.get("level");
            Object totalExpObj = request.get("totalExp");

            if (levelObj == null || totalExpObj == null) {
                Map<String, Object> result = new HashMap<>();
                result.put("success", false);
                result.put("message", "等级和经验值不能为空");
            ...
            try {
                ...
                if (totalExpObj instanceof Integer) {
                    totalExp = (Integer) totalExpObj;
                } else if (totalExpObj instanceof Long) {
                    totalExp = ((Long) totalExpObj).intValue();
                } else if (totalExpObj instanceof String) {
                    totalExp = Integer.parseInt((String) totalExpObj);
                } else if (totalExpObj instanceof Number) {
                    totalExp = ((Number) totalExpObj).intValue();
                }
            } catch (NumberFormatException e) {
                Map<String, Object> result = new HashMap<>();
                result.put("success", false);
                result.put("message", "等级和经验值必须是数字");
                result.put("timestamp", System.currentTimeMillis());
                return ResponseEntity.badRequest().body(result);
            }
            ...
            if (level == null || totalExp == null) {
                Map<String, Object> result = new HashMap<>();
                result.put("success", false);
                result.put("message", "等级和经验值格式错误");
                result.put("timestamp", System.currentTimeMillis());
                return ResponseEntity.badRequest().body(result);
            }

            // 3. 参数校验：等级必须 >= 1，经验值必须 >= 0
            if (level < 1) {
                Map<String, Object> result = new HashMap<>();
                result.put("success", false);
                result.put("message", "等级必须大于等于1");
                result.put("timestamp", System.currentTimeMillis());
                return ResponseEntity.badRequest().body(result);
            }
